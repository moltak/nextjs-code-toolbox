...
export default function JsonToCsv() {
  ...
  const handleConvert = () => {
    try {
      const jsonArray = Array.isArray(JSON.parse(json))
        ? JSON.parse(json)
        : [JSON.parse(json)];
      ...
      const rows = jsonArray.map((obj) =>
        headers
          .map((header) => (obj[header] !== undefined ? obj[header] : ""))
          .join(",")
      );
  ...
  return (
    <div className="min-h-screen bg-gray-100 p-8 text-black">
      ...
      <p className="text-center mb-6 text-gray-700">
        Paste your JSON data below and press <strong>Ctrl + Enter</strong> or click the convert button to generate CSV.
      </p>
      <textarea
